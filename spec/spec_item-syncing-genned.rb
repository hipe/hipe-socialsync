...
describe "Item syncing tests (generated tests)" do
  ...
  it "sosy users:add anonymous@sosy hipe@sosy (item-sync-2)" do
    x = @app.run(["users:add", "anonymous@sosy", "hipe@sosy"])
    y = "Created user \"anonymous@sosy\". Now there are 3 users."
    x.to_s.chomp.should.equal y
  end

  it "sosy accounts:add wordpress hipe@sosy wpuser1 (item-sync-3)" do
    x = @app.run(["accounts:add", "wordpress", "hipe@sosy", "wpuser1"])
    y = "Added wordpress account of \"wpuser1\"."
    x.to_s.chomp.should.equal y
  end

  it "sosy accounts:add wordpress hipe@sosy wpuser2 (item-sync-4)" do
    x = @app.run(["accounts:add", "wordpress", "hipe@sosy", "wpuser2"])
    y = "Added wordpress account of \"wpuser2\"."
    x.to_s.chomp.should.equal y
  end

  it "sosy accounts:add tumblr hipe@sosy chip.malice@gmail.com (item-sync-5)" do
    x = @app.run(["accounts:add", "tumblr", "hipe@sosy", "chip.malice@gmail.com"])
    y = "Added tumblr account of \"chip.malice@gmail.com\"."
    x.to_s.chomp.should.equal y
  end

  it "sosy accounts:add wordpress hipe@sosy wpuser3 (item-sync-6)" do
    x = @app.run(["accounts:add", "wordpress", "hipe@sosy", "wpuser3"])
    y = "Added wordpress account of \"wpuser3\"."
    x.to_s.chomp.should.equal y
  end

  it "# add item wordpress wpuser1 101 cat (item-sync-7)" do
  ...
  it "# add item wordpress wpuser1 102 dog (item-sync-8)" do
  ...
  it "# add item wordpress wpuser2 103 giraffe (item-sync-9)" do
  ...
  it "# add item wordpress wpuser3 (hipe@sosy) 201 hippo (item-sync-10)" do
  ...
  it "# add item tumblr chip (hipe@sosy) 901 camel (item-sync-11)" do
